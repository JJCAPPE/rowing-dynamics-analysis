...
from rowing.matching.pair_session import auto_pair_run
...
def _build_match_manifest(
    ...
    *,
    ...
    anchor_rp3_idx: int,
    cfg: MatchConfig,
    pinned_rp3_row_by_relative_idx: dict[int, int] | None = None,
    excluded_relative_indices: set[int] | None = None,
) -> MatchResult:
    """Build the RP3 match manifest via DP.

    ``pinned_rp3_row_by_relative_idx`` maps positions in the post-anchor video
    sequence (i.e. ``relative_i = video_stroke_idx - anchor_video_idx``) to
    the RP3 row index they must hit. ``excluded_relative_indices`` is the set
    of post-anchor positions to drop from the DP entirely (treated as gaps,
    consistent with the legacy behaviour of skipping rows that have no
    counterpart on the RP3 side).
    """
    ...
    excluded_relative_indices = set(excluded_relative_indices or set())
    if 0 in excluded_relative_indices:
        raise ValueError("Cannot exclude the anchor video stroke (relative_idx=0).")

    pinned_rp3_row_by_relative_idx = dict(pinned_rp3_row_by_relative_idx or {})
    pinned_overlap = set(pinned_rp3_row_by_relative_idx) & excluded_relative_indices
    if pinned_overlap:
        raise ValueError(
            f"Pinned and excluded video strokes overlap at relative_idx={sorted(pinned_overlap)}"
        )

    raw_v = video_df.iloc[anchor_video_idx:].reset_index(drop=True).copy()
    raw_v["__relative_idx"] = np.arange(len(raw_v), dtype=np.int64)

    if excluded_relative_indices:
        keep_mask = ~raw_v["__relative_idx"].isin(excluded_relative_indices)
        v = raw_v[keep_mask].reset_index(drop=True).copy()
    else:
        v = raw_v.copy()
    ...
    # Re-index pinned constraints into the (post-exclusion) DP coordinate i.
    pinned_by_dp_i: dict[int, int] = {}
    relative_to_dp_i = {int(rel): dp_i for dp_i, rel in enumerate(v["__relative_idx"].tolist())}
    for rel_i, rp3_row in pinned_rp3_row_by_relative_idx.items():
        if rel_i not in relative_to_dp_i:
            raise ValueError(f"Pinned video relative_idx={rel_i} is not reachable (excluded or out of range).")
        if not (0 <= int(rp3_row) < m):
            raise ValueError(f"Pinned rp3 row index {rp3_row} out of range [0, {m}).")
        pinned_by_dp_i[relative_to_dp_i[rel_i]] = int(rp3_row)

    # Anchor itself is implicitly pinned to anchor_rp3_idx; reject inconsistent overrides.
    if 0 in pinned_by_dp_i and pinned_by_dp_i[0] != anchor_rp3_idx:
        raise ValueError(
            "Pin on anchor video stroke must agree with anchor_rp3_idx="
            f"{anchor_rp3_idx} (got {pinned_by_dp_i[0]})."
        )
    ...
    for i in range(1, n):
        ...
        # Apply pin constraint after row i is filled: collapse the row to the
        # single pinned column so the path *must* visit it.
        pinned_j = pinned_by_dp_i.get(i)
        if pinned_j is not None:
            row = dp[i].copy()
            dp[i, :] = inf
            dp[i, pinned_j] = row[pinned_j]
            if not math.isfinite(dp[i, pinned_j]):
                raise RuntimeError(
                    f"Pinned (video_relative_idx={i}, rp3_row_idx={pinned_j}) is unreachable "
                    "with the current jump/cumulative-error tolerances."
                )
